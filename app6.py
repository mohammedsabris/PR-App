...
OPENROUTER_API_KEY = "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"  # Replace with environment variable in production
...
# def create_trend_chart(keyword, search_results):
#     # Create a dictionary to store keyword mentions per day
#     days = 30
#     today = datetime.now()
#     date_counts = { (today - timedelta(days=i)).strftime('%Y-%m-%d'): 0 for i in range(days, 0, -1) }

#     # Simulate publication dates (Since real results don’t have dates, we randomize for now)
#     for result in search_results:
#         rand_days_ago = random.randint(1, days)  # Simulate a recent date
#         date_str = (today - timedelta(days=rand_days_ago)).strftime('%Y-%m-%d')
#         date_counts[date_str] += 1  # Increase mention count on that day

#     # Convert dictionary to DataFrame
#     df = pd.DataFrame(list(date_counts.items()), columns=["date", "mentions"])
#     df = df.sort_values("date")

#     # Create the trend chart with Plotly
#     fig = px.line(df, x="date", y="mentions", title=f"Mentions of '{keyword}' over the last 30 days")
#     fig.update_layout(
#         xaxis_title="Date",
...
# Function to create a time series chart of trends
# def create_trend_chart(keyword):
...
#     # Create somewhat realistic trend data with some patterns
#     base = random.randint(50, 500)
    
#     # Add some weekly patterns and overall trend
#     trend = []
#     overall_direction = random.choice([-1, 1])  # Random overall trend direction
    
#     for i in range(days):
#         # Weekly pattern (weekends have different activity levels)
#         day_of_week = (today - timedelta(days=days-i-1)).weekday()
...
#         # Overall trend influence (increases over time)
#         trend_influence = overall_direction * (i/days) * 0.15
...
#         else:
#             next_val = trend[-1] * (1 + daily_change + trend_influence) * weekend_factor
#             trend.append(max(10, next_val))  # Ensure always positive
...
#     # Create the chart with Plotly
#     fig = px.line(df, x='date', y='interest', title=f'Interest in "{keyword}" over the last 30 days')
#     fig.update_layout(
#         xaxis_title='Date', 
#         yaxis_title='Interest Level',
#         hovermode='x unified'
...
if search_button and keyword:
    ...
    with st.spinner(f"Analyzing '{keyword}'..."):
        ...
        if not search_results:
            ...
            tab1, tab2, tab3, tab4,tab5 = st.tabs(["📈 Trends & Topics", "😊 Sentiment", "📋 Details","Reviews","🔍 Scraped Data"])
            ...
            with tab3:
                ...
                elif analysis and "key_metrics" in analysis and analysis["key_metrics"]:
                    # Show metrics as a table
                    metrics_df = pd.DataFrame(list(analysis["key_metrics"].items()), 
                                             columns=['Metric', 'Value'])
                    st.table(metrics_df)
                else:
                    st.write("No specific metrics found in the search results.")
                    ...
                    if reviews:
                        ...
                        for i, review in enumerate(reviews[:10]):  # Limit to 10 displayed reviews
                            with st.expander(f"Review {i+1} ({review['category']})"):
                                ...
                                st.caption(f"Sentiment score: {review['sentiment']:.2f}")
                    else:
